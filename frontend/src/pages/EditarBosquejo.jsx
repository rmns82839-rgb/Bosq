...
const EditarBosquejo = () => {
  ...
  const { bosquejos, updateBosquejo, setCurrentBosquejo, loadBosquejo } = useBosquejoStore();
  const [bosquejoData, setBosquejoData] = useState(null);
  ...
  useEffect(() => {
    ...
    const found = bosquejos.find(b => b.id === id);
    if (found) {
      setBosquejoData(found);
      setCurrentBosquejo(found);
      setIsLoading(false);
    } else {
      loadBosquejo(id).then((data) => {
        if (data) {
          setBosquejoData(data);
          setCurrentBosquejo(data);
        } else {
          toast.error('Bosquejo no encontrado');
          navigate('/bosquejos');
        }
        setIsLoading(false);
      });
  ...
  const handleSave = async () => {
    if (!bosquejoData) return;
    try {
      await updateBosquejo(id, bosquejoData);
  ...
  const renderPageContent = ({ pageId, pageComponent, pageIndex, isActive }) => {
    ...
    const removePunto = (idx) => {
      const currentPuntos = bosquejoData.puntos || [];
      if (currentPuntos.length <= 1) {
        toast.error('Debe haber al menos un punto');
        return;
      }
      const newPuntos = currentPuntos.filter((_, i) => i !== idx);
      setBosquejoData(prev => ({ ...prev, puntos: newPuntos }));
    };
    ...
    switch (pageComponent) {
      ...
      case 'punto':
        const idx = pageIndex;
        const punto = (bosquejoData.puntos && bosquejoData.puntos[idx]) || { titulo: '', descripcion: '', versos: '' };
            ...
            <div className="page-content" style={{ overflow: 'hidden', flex: 1 }}>
              <div style={{ flex: 1, overflowY: 'auto', paddingRight: '5px' }}>
                <div id="finalDocument" className="final-document">
                  ...
                  {(bosquejoData.puntos && bosquejoData.puntos.length > 0) && (
                    <>
  ...
  if (!bosquejoData) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <p className="text-gray-600 dark:text-gray-400">Bosquejo no encontrado</p>
      </div>
    );
  }
