...
export default function Neumatologia() {
  ...
  useEffect(() => {
    ...
    let promise
    if (key === 'nombres') promise = getNombresEspiritu()
    else if (key === 'obras') promise = getObrasEspiritu()
    else if (key === 'simbolos') promise = getSimbolosEspiritu()
    else if (key === 'dones') promise = getDonesEspiritu()
    else if (key === 'fruto') promise = getFrutoEspiritu()
    else if (key === 'porLibro') promise = getEspirituPorLibro()

    promise.then(d => { setData(prev => ({ ...prev, [key]: d })); setLoading(false) })
  ...
  function irA(libroId, capNum) {
    if (libroId && capNum) navigate(`/biblia/leer/${libroId}/${capNum}`)
  }
